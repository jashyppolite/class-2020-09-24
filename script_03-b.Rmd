...
```{r}
classes_inc <- inner_join(sep_old, sep_new, by = c("id", "title", "name", "department")) %>%
...
```{r}
classes_dec <- inner_join(sep_old, sep_new, by = c("id", "title", "name", "department")) %>%
  mutate(change = (new_u_grad - old_u_grad)) %>%
  arrange((change)) %>%
  slice(1:5)
...
classes_dec %>% gt() %>%
  ...
  cols_label(title = "Number",
             name = "Title",
             new_u_grad = "Enrollment")
```

c) Make a nice looking graphic which shows these 10 courses.
```{r}
total <- rbind(classes_big_dec, classes_big_inc)

ggplot(total, aes(x = name, y = change, fill = department)) + geom_col() +
